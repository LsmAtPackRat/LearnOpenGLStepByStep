...
<u>所以说，VAO从CPU被发送到显存，使用VBO管理，然后GPU从显存中直接访问VBO。</u>


顶点缓冲对象（VBO）就像OpenGL中的其它对象一样，有一个独一无二的ID，所以我们可以使用**glGenBuffers**函数和一个缓冲ID生成一个VBO对象：
...
如果我们打算做渲染的话，现代OpenGL需要我们至少设置一个**顶点着色器**和一个**片段着色器**，这两个着色器也是我们能够设置的唯二两个着色器。

顶点着色器的使用流程大致如下：

​	使用GLSL编写顶点着色器—>编译顶点着色器—>使用顶点着色器。
